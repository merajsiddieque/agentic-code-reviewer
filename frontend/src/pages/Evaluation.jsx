...
export default function Evaluation() {
  ...
  return (
    <div className="space-y-8 animate-fadeIn">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-6 border-b border-[#1F2937]">
        ...
        <div className="flex items-center gap-3">
          <button
            ...
            disabled={isLoading}
            className="inline-flex items-center gap-2 px-3.5 py-2 rounded-xl text-xs font-semibold bg-[#111827] hover:bg-[#1E293B] text-zinc-200 border border-[#1F2937] transition disabled:opacity-50"
          >
      ...
      {/* Failed Cases Panel */}
      <div className="rounded-2xl bg-[#111827] border border-[#1F2937] p-6 sm:p-7 shadow-xl backdrop-blur-md">
          ...
          <div className="space-y-4">
            {failedCases.map((fCase, idx) => {
              ...
              return (
                <div
                  key={idx}
                  className="rounded-xl border border-rose-900/80 bg-[#0B1120] p-5 space-y-3"
                >
                  <div
                  ...
                  {isExpanded && (
                    <div className="pt-3 border-t border-[#1F2937] space-y-3 text-xs">
                      <div>
                        <span className="font-semibold text-zinc-300">Expected Findings:</span>
                        <div className="flex flex-wrap gap-1.5 mt-1">
                          {fCase.expected_findings?.map((ef, i) => (
                            <span key={i} className="px-2 py-0.5 rounded bg-[#111827] text-zinc-300 font-mono border border-[#1F2937]">
                              {ef}
                            </span>
                          ))}
                        </div>
                      ...
                      {fCase.review_output && (
                        <div>
                          ...
                          <pre className="mt-1 p-3 rounded-lg bg-[#111827] text-zinc-300 font-mono text-[11px] overflow-x-auto max-h-48 border border-[#1F2937]">
                            {fCase.review_output}
                          </pre>
                        </div>
                      )}
