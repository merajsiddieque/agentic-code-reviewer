...
export default function UploadCard({
  ...
  const handleValidateAndSelect = (file) => {
    ...
    if (!isSupported) {
      setValidationError(
        'Unsupported file type. Supported formats: .py, .js, .jsx, .ts, .tsx, .java, .html, .css, .md, .json, .yml, .yaml'
      );
      return;
    }

    if (file.size === 0) {
      setValidationError('The selected file is empty. Please select a valid source code or configuration file.');
      return;
    }
  ...
  return (
    <div className="relative overflow-hidden rounded-2xl bg-zinc-900/90 border border-zinc-800 shadow-xl backdrop-blur-md transition-all duration-300">
      ...
      <div className="p-6 sm:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
          ...
          <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-mono bg-zinc-800 text-zinc-300 border border-zinc-700">
          ...
          <div
            ...
            className={`group relative border-2 border-dashed rounded-2xl p-8 sm:p-12 text-center cursor-pointer transition-all duration-300 ${
              isDragging
                ...
                : 'border-zinc-700/80 bg-zinc-950/50 hover:border-indigo-500/70 hover:bg-zinc-950/80 hover:shadow-lg'
            }`}
          >
            <input
              ref={fileInputRef}
            ...
            <div className="mx-auto w-16 h-16 rounded-2xl bg-zinc-900 border border-zinc-800 text-indigo-400 flex items-center justify-center mb-4 group-hover:scale-110 group-hover:border-indigo-500/50 group-hover:text-indigo-300 transition-all duration-300 shadow-inner">
            ...
            <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-md text-xs font-mono text-zinc-400 bg-zinc-900 border border-zinc-800">
              <span>.py, .js, .jsx, .ts, .tsx, .java, .html, .css, .md, .json, .yml, .yaml</span>
            </div>
          </div>
        ) : (
          <div className="rounded-2xl border border-zinc-700 bg-zinc-950/70 p-5 backdrop-blur shadow-inner">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
              <div className="flex items-center space-x-4">
                <div className="w-12 h-12 rounded-xl bg-indigo-950 border border-indigo-800 text-indigo-400 flex items-center justify-center flex-shrink-0 shadow-sm">
              ...
              <div className="flex items-center gap-2 self-end sm:self-center">
                <button
                  ...
                  onClick={() => fileInputRef.current?.click()}
                  disabled={isLoading}
                  className="px-3.5 py-1.5 text-xs font-medium text-indigo-300 bg-indigo-950/70 hover:bg-indigo-900/80 border border-indigo-700/80 rounded-lg transition-colors disabled:opacity-50"
                >
                  Replace File
                </button>
                <input
                ...
                <button
                  ...
                  className="p-1.5 text-zinc-400 hover:text-rose-400 hover:bg-rose-950/60 rounded-lg transition-colors disabled:opacity-50 border border-transparent hover:border-rose-900"
                >
        ...
        <div className="mt-6 flex flex-col sm:flex-row items-center justify-between gap-4">
          ...
          <button
            ...
            className={`w-full sm:w-auto inline-flex items-center justify-center gap-2.5 px-7 py-3 rounded-xl font-semibold text-sm shadow-xl transition-all duration-300 ${
